...
public class companyImpl implements CompanyService{
	...
	public CompanyResponse addCompany(final CompanyRequest userRequest) {
		final CompanyEntity companyData = requestConverter.toUserEntity(userRequest);
...
//	}
	...
	public CompanyResponse updateCompany(CompanyRequest companyRequest,Long id) {
	    Optional<CompanyEntity> companyData = companyRepository.findById(BigInteger.valueOf(id));
	    if(companyData.isPresent()) {
	        ...
	        companyEntity.setDeleteCheck(companyRequest.getDeleteCheck());
